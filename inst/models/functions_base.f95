...
!! current known differences to original rSalmo C code:
!! - we do not calculate phytoplankton in multiple depths, but just the middle
!! for this we use an average light formula
!! - we assume permanent saturation with oxygen due to mixing and shallow lake
...
function f_apsf (temp, o, n, npsfmode, APSFMAX, APSFMIN, KAPSF, LINDEN, APSFT) result (res)
    ...
    double precision:: klinden, oplusn, apsf_result, res

    IF (npsfmode == 1) THEN ! 4.10
        klinden = 0.3 * LINDEN ! release only dependent on N
    
        IF (N <= klinden) THEN
            apsf_result = APSFMAX
        ELSE
            apsf_result = APSFMAX /(N-klinden) /(1 /(KAPSF - klinden) + 1/(N - klinden)) + APSFMIN
        END IF
    ELSE
        oplusn = o + n / 0.3 ! release dependent on N and O2
    
        IF (oplusn <= LINDEN) THEN
            apsf_result = APSFMAX 
        ELSE
            apsf_result = APSFMAX /(oplusn - LINDEN) /(1 /(KAPSF - LINDEN) + 1/(oplusn - LINDEN)) + APSFMIN
        END IF
    END IF
